...
### 2. **Backend (Express.js + PostgreSQL + PostGIS)**
- **Database**: PostgreSQL 17 with PostGIS spatial extensions
- **API Server**: Express.js server handling database queries and authentication
- **Functions**: Spatial query functions for map data and location services
- **Auth**: Session-based authentication with player/DM/admin roles
...
#### **Current Architecture: Local PostgreSQL with Express.js**
- **Database**: PostgreSQL 17 with PostGIS extensions
- **API Server**: Express.js server on port 3001
- **Authentication**: Session-based with bcrypt password hashing
- **Environment**: `VITE_DATABASE_SERVER_URL=http://localhost:3001` (or your HTTPS endpoint)

#### **Production Options:**
- **Cloud Managed**: AWS RDS, Google Cloud SQL, Azure Database
- **Self-Hosted**: Docker containers, VPS with PostgreSQL
- **Hybrid**: Local development → Cloud production

### Local Development Setup

**Database Configuration**:
- Uses `VITE_DATABASE_SERVER_URL` environment variable
- Connects to Express.js server which handles PostgreSQL queries
- Console logging indicates database connection status

**Mock Data Fallback**: 
- When database server is unavailable, components use mock data
...
- Single database client connects to Express.js server
- Lazy-loaded database client prevents environment issues
...
# Start database server (in one terminal)
cd server && npm start
...
- Express.js API server for application logic
- Session-based authentication with secure cookies

**Self-Hosted PostgreSQL**:
- PostgreSQL 17 server with PostGIS extension
- Express.js application server on port 3001
- Docker support for containerized deployment
- Full control over database and application logic
...
- `/utils/database/client.tsx` - PostgreSQL database client
- `/utils/database/data-helpers.tsx` - Database query helpers
- `/utils/database/data-structures.tsx` - TypeScript interfaces
- `/database/schema.sql` - PostGIS database schema with spatial functions
- `/server/database-server.js` - Express.js API server
