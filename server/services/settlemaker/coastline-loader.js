...
//
// The settlement view should show what the world map shows at high zoom:
// the burg's OWN adjacent water body dominating one side of the town, not
// a 3000 km survey of every pond in the region squeezed into the frame
// (burg 15953 "Ertelenlik": its shoreline lake rendered as a small blob
// among far-away ponds until this radius was cut from 3000 km to 400 km).
//
// - Floor (20 px) guarantees a few cells of shape on coarse worlds.
// - Target (400 km) ≈ 40 px on Snoopia (10 km/px) — the width of a typical
//   burg-adjacent lake, so its shore crosses the settlement frame.
// - Ceiling (500 px) caps cell-count on ultra-fine worlds where the meters
//   target would balloon into thousands of cells.
const MIN_R_WORLD_PX = 20;
const MAX_R_WORLD_PX = 500;
const TARGET_R_WORLD_METERS = 400_000;
...
export async function loadCoastlineGeometry(client, burg, opts = {}) {
  ...
  // ST_ChaikinSmoothing rounds the jagged Voronoi cell edges so the
  // settlement shoreline matches the world map's smoothed coast curves.
  const { rows } = await client.query(
    `WITH b AS (SELECT geom FROM public.maps_burgs WHERE id = $1)
     SELECT ST_AsGeoJSON(ST_ChaikinSmoothing((ST_Dump(ST_Union(c.geom))).geom, 3)) AS poly
