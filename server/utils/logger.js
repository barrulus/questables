...
export const logInfo = (message, meta) =>
  console.info(fmt("info", message, meta));
export const logHttp = (message, meta) =>
  console.log(fmt("http", message, meta));
export const logDebug = (message, meta) =>
  console.debug(fmt("debug", message, meta));

// Align server-side DB logging with pool usage (object payload)
export const logDatabaseOperation = ({ text, duration, rowCount, label }) => {
  logInfo("Database query", {
    label,
    duration: duration ? `${duration}ms` : undefined,
    rowCount,
    text: typeof text === "string" ? text.slice(0, 120) : undefined,
  });
};
...
export const logSecurityEvent = (event, severity = "low", details) => {
  const logger = severity === "high" || severity === "critical"
    ? logError
    : logWarn;
  logger(`Security event: ${event}`, {
    severity,
    details,
    timestamp: new Date().toISOString(),
  });
...
export const logApplicationStart = (config) =>
  logInfo("Database server starting", config);
export const logApplicationShutdown = (reason) =>
  logWarn("Database server stopping", { reason });
